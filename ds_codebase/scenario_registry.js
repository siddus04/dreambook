...
(function (global) {
    ...
    /** Multi-paragraph-safe quality gate for band_full widget source text. */
    function isUsableBandTeachingPassage(text) {
        if (!text?.trim()) return false;
        if (isPedagogyChrome(text)) return false;
        if (/progress_activity|Generating widget content/i.test(text)) return false;
        if (/author-widget-pending|animate-spin/i.test(text)) return false;
        ...
        if (isPedagogyChrome(cleaned)) return false;
        const paragraphs = cleaned.split(/\n\n+/).map(p => p.trim()).filter(Boolean);
        const proseParagraphs = paragraphs.filter(p => isTeachingProseParagraph(p));
        const proseChars = proseParagraphs.join(' ').length;
        return proseChars >= 40;
    }
    ...
    /** Full major-band text for widget generation (preserves intro + subsections). */
    function extractBandTeachingPassage(text, maxLen) {
        const cleaned = stripPedagogyChrome(text);
        if (!cleaned) return '';
        maxLen = maxLen || 2800;
        const paragraphs = cleaned.split(/\n\n+/).map(p => p.trim()).filter(Boolean);
        const teachingParts = paragraphs.filter(p => {
            if (/^#{1,3}\s/.test(p)) return true;
            if (isTeachingProseParagraph(p)) return true;
            return p.length >= 30 && !isPedagogyChrome(p);
        });
        let excerpt = teachingParts.join('\n\n');
        if (!excerpt) excerpt = cleaned;
        return excerpt.length <= maxLen ? excerpt : excerpt.slice(0, maxLen - 1) + '…';
    }
    ...
    global.DreamBookScenarioRegistry = {
        ...
        isUsableBandTeachingPassage,
        extractTeachingExcerpt,
        extractBandTeachingPassage,
