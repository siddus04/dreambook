...
Unless AUTHOR NOTE specifies a domain, choose the analogy domain dynamically — factory, kitchen, school, library, delivery network, office, construction site, hospital, farm, or another familiar setting. When AUTHOR NOTE or AUTHOR DOMAIN (MANDATORY) is present, follow it exactly and do not substitute another setting. Pick the best fit; do not default to city or generic house unless it truly fits.
...
Rules: Use the SOURCE PASSAGE as truth — name at least two specific terms from it in THE ANALOGY. Identify key technical terms or mechanisms in the passage; map as many as are necessary to explain the core concept clearly, prioritizing the most important ones. Use at least 2 specific terms when available. If the passage contains fewer than 2 technical terms, map the main concept and one supporting detail instead. Map relationships (e.g. information flow, compartment roles), not parallel lists. If the passage compares two categories or architectures, explain BOTH sides in THE ANALOGY — do not generate an analogy that covers only one category. If the passage mentions mRNA translation, ribosomes read messenger RNA (mRNA) to build proteins — do NOT call rRNA a protein blueprint. The nucleolus helps make/assemble ribosomal RNA and ribosome parts. Do NOT use concerts, bands, phone apps, Lego cities, superheroes, or generic chef-kitchen clichés. Do NOT start with "Imagine".`
        },
        news: {
            id: 'illustrate-news',
...
Verification status:`
    ...
    function buildMockRecallPayloadForIndex(format, sourceText, typeIndex, learningContext) {
        // Dev/manual fallback only — not for chapter_enhancements hydration.
    ...
    function buildRecallBundlePrompt(config, sourceText, learningContext) {
        ...
        const orderDistinctNote = (validated.order || 0) > 1
            ? '\nFor multiple Order exercises, each must use a distinct prompt and a different set of steps from separate processes in the passage. Do not repeat prompts or step lists.'
            : '';
...
Return JSON:
{
  "exercises": [
    ...
    function buildRecallSingleFormatConfig(format) {
        const f = String(format || 'mcq').toLowerCase();
        return {
            mcq: f === 'mcq' ? 1 : 0,
            match: f === 'match' ? 1 : 0,
            order: f === 'order' ? 1 : 0
    ...
    function generateRecallExercisePrompt(format, sourceText, learningContext) {
        return buildRecallBundlePrompt(
            buildRecallSingleFormatConfig(format),
            sourceText,
            learningContext
        );
    }

    function parseRecallSingleExercise(raw, format) {
        const config = buildRecallSingleFormatConfig(format);
        const normalized = normalizeRecallBundle(raw, config);
        const fmt = String(format || 'mcq').toLowerCase();
        const exercise = normalized.find(ex => ex.format === fmt) || normalized[0];
        return exercise?.payload || null;
    }

    function normalizePassageKey(text) {
        return normalizeText(text).toLowerCase();
    }

    function fragmentAppearsInPassage(fragment, passageKey) {
        const key = normalizeText(fragment).toLowerCase();
        if (!key || key.length < 3) return false;
        return passageKey.includes(key);
    }

    function recallPayloadGroundedInPassage(payload, sourceText) {
        if (!payload || !sourceText?.trim()) return false;
        const passageKey = normalizePassageKey(sourceText);
        const format = String(payload.format || '').toLowerCase();

        if (format === 'match') {
            const pairs = payload.pairs || [];
            if (pairs.length < 2) return false;
            const grounded = pairs.filter(p => fragmentAppearsInPassage(p.term, passageKey));
            return grounded.length >= 2;
        }
        if (format === 'order') {
            const steps = payload.steps || [];
            if (steps.length < 2) return false;
            const grounded = steps.filter(s => fragmentAppearsInPassage(s, passageKey));
            return grounded.length >= 2;
        }
        if (format === 'mcq') {
            ...
            const combined = [q.prompt, correct, q.explanation].filter(Boolean).join(' ');
            const words = combined.match(/\b[a-z]{4,}\b/gi) || [];
            const hits = words.filter(w => passageKey.includes(w.toLowerCase()));
            return hits.length >= 2;
        }
        return false;
    }

    function getRecallExerciseFingerprint(exercise) {
        if (!exercise?.payload) return '';
        const p = exercise.payload;
        const fmt = String(exercise.format || p.format || '').toLowerCase();
        if (fmt === 'mcq') {
            const q = p.questions?.[0];
            return 'mcq:' + normalizeText(q?.prompt || '');
        }
        if (fmt === 'match') {
            const terms = (p.pairs || []).map(pair => normalizeText(pair.term)).sort().join('|');
            return 'match:' + terms;
        }
        if (fmt === 'order') {
            const prompt = normalizeText(p.prompt || '');
            const steps = (p.steps || []).map(s => normalizeText(s)).join('|');
            return 'order:' + prompt + '::' + steps;
        }
        return fmt + ':' + JSON.stringify(p);
    }

    function dedupeRecallExercises(exercises) {
        const seen = new Set();
        const out = [];
        (exercises || []).forEach(ex => {
            const fp = getRecallExerciseFingerprint(ex);
    ...
    function getRecallPromptText(payload) {
        ...
        if (format === 'mcq') {
            return data.questions?.[0]?.prompt || data.questions?.[0]?.question || data.prompt || data.question || '';
        }
        if (format === 'match') {
            return data.prompt || 'Match each term to its definition.';
        }
        if (format === 'order') {
            return data.prompt || 'Put the steps in the correct order.';
        }
        return '';
    }

    function getRecallQuestionHeading(format, typeIndex, payload) {
        const idx = Math.max(1, parseInt(typeIndex, 10) || 1);
        const fmt = format || (typeof payload === 'object' ? payload?.format : '') || '';
        const prompt = getRecallPromptText(payload || { format: fmt });
        if (prompt) return `${idx}. ${prompt}`;
        return getRecallTypeSublabel(fmt, idx);
    }

    function getRecallTypeSublabel(format, typeIndex) {
        const idx = Math.max(1, parseInt(typeIndex, 10) || 1);
        if (format === 'mcq') return `${idx}. Question ${idx}`;
        if (format === 'match') return `${idx}. Match ${idx}`;
        if (format === 'order') return `${idx}. Order steps ${idx}`;
        const prefix = RECALL_TYPE_LABELS[format] || String(format);
    ...
    function parseExplainPeerWhiteboardContent(content) {
        const mf = global.DreamBookModuleFormats;
        return mf?.parseExplainPeerWhiteboardContent?.(content) || { setup: '', task: '', structured: false };
    }

    function parseExplainPeerEli5Content(content) {
        const mf = global.DreamBookModuleFormats;
        return mf?.parseExplainPeerEli5Content?.(content) || { coreIdea: '', context: '', structured: false };
    }

    function renderRecallStudentHtml(blockId, payload, status, recallResult, opts) {
        opts = opts || {};
        const hidePrompt = opts.hidePrompt === true;
        ...
        if (data.format === 'mcq') {
            ...
            return `<div class="rc-body" data-recall-id="${blockId}">
                ${hidePrompt ? '' : `<p class="rc-prompt rc-prompt-bold">${escapeHtml(q.prompt)}</p>`}
                <div class="rc-mcq-options">${optionsHtml}</div>
                ${summaryHtml}
                ${renderFeedback()}
        ...
        if (data.format === 'match') {
            ...
            return `<div class="rc-body rc-match" data-recall-id="${blockId}">
                ${hidePrompt ? '' : `<p class="rc-prompt rc-prompt-bold">${escapeHtml(data.prompt || 'Match each term to its definition.')}</p>`}
        ...
        if (data.format === 'order') {
            ...
            const items = orderedItems.map(({ text, correctIndex }, i) => {
                const draggable = isFinished ? 'false' : 'true';
                return `<li class="rc-order-item" draggable="${draggable}" data-correct-index="${correctIndex}"><span class="rc-order-num">${i + 1}.</span><span class="rc-order-text">${escapeHtml(text)}</span></li>`;
            }).join('');
            ...
            return `<div class="rc-body rc-order" data-recall-id="${blockId}">
                ${hidePrompt ? '' : `<p class="rc-prompt rc-prompt-bold">${escapeHtml(data.prompt || 'Put the steps in the correct order.')}</p>`}
    ...
    global.DreamBookEnhancements = {
        ...
        buildRecallSingleFormatConfig,
        generateRecallExercisePrompt,
        parseRecallSingleExercise,
        recallPayloadGroundedInPassage,
        normalizeRecallBundle,
        getRecallExerciseFingerprint,
        dedupeRecallExercises,
        getRecallTypeSublabel,
        getRecallQuestionHeading,
        getRecallPromptText,
        RECALL_FORMAT_ORDER,
        RECALL_TYPE_HEADERS,
        ...
        makeProposal,
        sectionLooksDefinitional,
        sectionLooksSequential,
