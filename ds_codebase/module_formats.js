...
Rules: Do NOT use "Model A" / "Model B". Do NOT start with "Imagine". Name at least two specific terms from the SOURCE PASSAGE in the claim or counter-view. Do NOT include a QUESTIONS section — the student exercise uses a fixed Socratic ladder.`,
                    openingPromptGen: `Return ONLY this exact question verbatim — no preamble: What part of the claim sounds reasonable?`,
                    hintTemplate: 'Start with what sounds reasonable in the claim, then find a counterexample and repair the overgeneralization.',
                    goalTemplate: 'Reason through a flawed claim using a Socratic ladder — useful truth, counterexample, mechanism, and repaired claim.',
...
Takeaway: (one sentence)`,
    ...
    const DEBATE_LADDER_LABELS = ['Useful truth', 'Counterexample', 'Repair claim'];
    const DEBATE_CANONICAL_PROMPTS = [
        'What part of the claim sounds reasonable?',
        'What kind of cell challenges the idea that all cells need a nucleus?',
        'Rewrite the claim so it becomes scientifically accurate.'
    ];
    ...
    function buildDebateQuestionLadder() {
    ...
    function createDebateSocraticState(stepNum) {
    ...
    function buildDebateCoachVisibleReply(stepEval, fallbackText) {
        ...
        let visible = fb || fallback;
        if (stepEval && stepEval.should_advance) {
        ...
        if (nudge) {
            if (visible && !visible.includes('?')) {
                visible = `${visible.replace(/\.\s*$/, '')}. ${nudge}`;
            } else if (!visible) {
                visible = nudge;
            }
        }
        return visible.trim() || fallback;
    }
    ...
    function evaluateDebateStepAnswer(step, studentText, state) {
        ...
        if (n === 1) {
            const goodEnough = /nucleus.*(control|activit)|control.*(cell|activit)|nucleus.*(important|store|organiz)|stores dna|genetic material|eukaryot.*nucleus|nucleus helps/i.test(t)
                || (/(nucleus|control|activit|important|dna|genetic)/i.test(t) && words.length >= 3);
            if (goodEnough) {
                ...
                result.nudge_question = 'Close. What does the nucleus contain or organize that helps the cell control activities?';
            }
            return result;
        }
        if (n === 2) {
        ...
        if (n === 3) {
            ...
            const hasRepair = words.length >= 8
                && (/eukaryot|nucleus/i.test(t))
                && (/prokaryot|bacteri|archaea|no nucleus|without.*nucleus|lack.*nucleus/i.test(t))
                && (/nucleoid|dna|genetic|not all|without|lack/i.test(t));
            if (hasRepair) {
                result.is_correct = true;
                result.is_complete_for_current_step = true;
                result.should_advance = true;
                result.feedback = 'Complete — that captures the useful truth, the counterexample, and the corrected claim.';
                known.repaired_claim = true;
                result.known_concepts = known;
                return result;
            }
            if (partialOnly || words.length < 8) {
                result.is_correct = true;
                result.is_complete_for_current_step = false;
                result.missing_piece = 'contrast between eukaryotic and prokaryotic cells';
                result.nudge_question = 'Good start. Complete this: "Eukaryotic cells ___, but prokaryotic cells ___, so ___."';
                result.known_concepts = known;
                return result;
            }
            result.missing_piece = 'a repaired claim with both eukaryotic and prokaryotic contrast';
            result.nudge_question = 'Good start. Complete this: "Eukaryotic cells ___, but prokaryotic cells ___, so ___."';
            result.known_concepts = known;
            return result;
        }
    ...
    function mergeDebateStepEvaluations(coachEval, clientEval) {
        ...
        const feedback = (client.feedback && clientComplete)
            ? client.feedback
    ...
    function updateDebateSocraticState(state, step, mergedEval, studentText) {
        ...
        if (mergedEval.should_advance) {
            ...
            if (n === 3) next.known_concepts.repaired_claim = true;
        } else {
    ...
    function resetDebateSocraticStateForStep(stepNum) {
        const step = parseInt(stepNum, 10) || 1;
        const next = createDebateSocraticState(step);
    ...
    function getDebateStepFooterCopy(step, stepStatus, stepsDone) {
        ...
        const labels = {
            1: 'identify what sounds reasonable in the claim',
            2: 'find the counterexample and how those cells still function',
            3: 'repair the claim precisely'
        };
        const activeGoal = labels[n] || 'keep reasoning';
        if (status === 'needs_nudge') {
            const nudgeHints = {
                1: 'close — name what the nucleus contains or organizes',
                2: 'close — name the cell type or where genetic material is found',
                3: 'close — add the eukaryotic vs prokaryotic contrast'
            };
    ...
    function buildDebateRubricCoachInstruction(opts) {
        ...
        const lines = [
            ...
            '- The UI shows the current step question in the prompt box. Do NOT repeat that question in your visible reply.',
            '- When the step is complete: return feedback ONLY in visible text. Set nudge_question to null. Do NOT ask the next step question — the UI advances and shows it.',
            '- When the step is incomplete: return brief feedback plus exactly ONE nudge_question in STEP_EVAL. At most one question visible per turn.',
            '',
            ...
            '- Step 1 Useful truth: nucleus controls activities / is important / stores DNA for eukaryotic cells.',
            '- Step 2 Counterexample (+ mechanism folded in): prokaryotes/bacteria/archaea lack a nucleus; genetic material is in a nucleoid. Reject bare "not all cells need a nucleus" without a cell type.',
            '- Step 3 Repair claim: student rewrites the claim with eukaryotic nucleus role, prokaryotic lack of nucleus, DNA/nucleoid, and that a nucleus is not required for all life.',
            '',
            'Step 3 partial answers: use a sentence frame — "Good start. Complete this: Eukaryotic cells ___, but prokaryotic cells ___, so ___." Do NOT restart the ladder.',
            '',
