...
(function (global) {
    ...
    function ensureMajorBandBundleCoverage(bands, proposals, menuById, outlineMeta, diagnosisByBand) {
        ...
        const usedBlocks = new Set((proposals || []).map(p =>
            `${p.afterBlockId}:${p.bundleTier || (isMFeatureKey(getProposalFeatureKey(p)) ? 'm' : 'non_m')}`
        ));
        ...
        (bands || []).forEach(band => {
            ...
            const hasNonM = bandProposals.some(p => p.bundleTier === 'non_m' || !isMFeatureKey(getProposalFeatureKey(p)));
            if (hasNonM) return;

            let weapon = inferWeaponFromDiagnosis(diag, band, usedFeatures);
            const weaponKey = weapon ? Object.entries(ENHANCEMENT_WEAPONS).find(([, w]) => w === weapon)?.[0] || '' : '';
            if (isMFeatureKey(weaponKey)) {
                weapon = downgradeWeaponFromSimulation(band, usedFeatures);
            }
            let downgradeNote = '';
            ...
            const featureKey = Object.entries(ENHANCEMENT_WEAPONS).find(([, w]) => w === weapon)?.[0] || 'socratic';
            const proposal = buildBandProposal({
                band,
                blockMeta,
                weapon,
    ...
    function assignChapterFormats(proposals, eligibleBands) {
        ...
        if (needNews > 0) {
            ...
            const candidates = results.filter(p =>
                (getProposalFeatureKey(p) === 'illustrate_analogy' && !analogyShaped(p))
                || p.scanId === 'scan_case_study'
            );
            candidates.slice(0, needNews).forEach(p => {
                if (!p.plannedFeatureKey) p.plannedFeatureKey = getProposalFeatureKey(p);
                if (!p.plannedScanId) p.plannedScanId = p.scanId;
                const fromKey = p.plannedFeatureKey;
                ...
                p.downgradeNote = p.downgradeNote || `Converted from ${fromKey} to news for chapter variety.`;
                newsCount++;
            });
        }
    ...
    function finalizeBandProposals(proposals, eligibleBands, diagnosisByBand) {
        ...
        return assignChapterFormats(results, eligibleBands);
    }
